...
**Correction (found after this fix was implemented, deployed, and re-measured against production):** the claim above was wrong. The grep that "confirmed" no other `@font-face` existed for `bootstrap-icons` missed the fact that `home.css`, `content.css`, and `tour.css` each carry their *own* duplicated `@font-face` declaration for `bootstrap-icons` — not just the selector rule. A real Lighthouse re-run against production after deploying this fix (per the Testing section below) showed **no measurable change to LCP at all**, which prompted a root-cause investigation instead of accepting the original single-file fix as complete. That investigation found **14 total occurrences** of a duplicated `bootstrap-icons` `@font-face` across the 3 critical CSS files — including 2 occurrences in `content.css` that pointed directly at a third-party CDN rather than the self-hosted font, meaning even a page loading `content.css`'s critical CSS was still eagerly fetching the full, unsubsetted webfont regardless of the `bootstrap-icons.min.css` fix. All 14 occurrences were corrected in a Round-2 fix (commit `b2e0aa13`), re-pointing them at the same `bootstrap-icons-subset.woff2` file. The lesson: a single grep pattern that matches the *obvious* declaration site is not sufficient proof of exhaustiveness when critical-CSS files are known (per this same codebase's own history — see the `fontello`/`icon_set_1` fix above) to duplicate `@font-face` blocks independently. See `includes/head.php`'s regeneration-instructions comment for the durable warning this produced.
