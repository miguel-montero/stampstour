...
**A static-markup-only scan isn't sufficient, and this was caught before it shipped, not after.** `js/functions.js` (lines 88 and 95) dynamically toggles `.toggleClass('icon-minus icon-plus')` on an accordion's chevron indicator — neither class appears anywhere in static PHP markup at all (confirmed via a repo-wide grep restricted to first-party code, excluding vendored plugin bundles which can't reference this site's own icon classes). A JS-only usage scan (`grep` for `toggleClass`/`addClass`/`classList.add` combined with `icon-`/`icon_set_1_icon-` across first-party `.js` files and inline `<script>` blocks in `.php` files) found exactly these 2 additional glyphs and nothing else. The final, confirmed, precise codepoint lists (36 fontello, 18 icon_set_1):
