...
- **`admin/_hero.php` is out of scope.** It uses the identical `.background-image`/`data-background` mechanism, but it's behind login, not performance-critical, and not what the Lighthouse audit measured. Leaving it means `js/functions.js`'s `.background-image` JS handler (lines 402-403) must stay in the codebase — it's still needed for that one page. This is intentional, not leftover debt from this change.
...
3. Confirm `js/functions.js`'s `.background-image` handler still works correctly for `admin/_hero.php` (the one remaining consumer) — log in to the local admin panel and check its hero section still renders.
