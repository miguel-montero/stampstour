...
- Exact changes already made, verified, and present in the working tree:
  ...
  - `css/vendors.unminified.css`: two line ranges deleted — lines 15858–16356 (Range Slider + Owl Carousel, consecutive) and lines 17213–17348 (Bootstrap Time picker). File went from 17,761 to 17,126 lines (after also removing a stale `@import` — see below). Table of contents comment at the top of the file updated to remove references to the 3 deleted sections and note why.
...
Expected: all `node --check` calls report no errors. `common_scripts.js` is 11,170 lines; `vendors.unminified.css` is 17,126 lines. `common_scripts_min.js` is 207,812 bytes; `vendors.css` is 214,195 bytes. The `ionRangeSlider`/`owlCarousel` grep returns `0`. The `.irs-`/`.owl-` grep returns `0`. The `.mfp-`/`.daterangepicker`/`.sp-slides` grep returns a large positive number (hundreds).
